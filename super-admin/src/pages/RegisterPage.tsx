...
export function RegisterPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-surface-app px-4 text-center">
        <p className="text-sm text-zinc-400">
          This page needs an invitation link. Check the email you were sent, or ask an existing Super Admin to resend it.
        </p>
      </div>
    );
  }
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await registerFromInvite({ token, name, password, bio });
    ...
    <div className="flex min-h-screen items-center justify-center bg-surface-app px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={onSubmit} className="rounded-xl2 border border-surface-border bg-surface-card p-6 shadow-xl">
          <h1 className="mb-1 text-lg font-semibold text-zinc-100">Accept your Super Admin invitation</h1>
          <p className="mb-6 text-sm text-zinc-500">Set up your account to join the platform's admin team.</p>
